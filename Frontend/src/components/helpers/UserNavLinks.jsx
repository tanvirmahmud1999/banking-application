  ...
  const navbarLinks = userNavData.map((link) => (
    <li
      ...
      className={`${
        ...
        "hidden"
      } flex w-full justify-between items-center mb-6 select-none hover:bg-blue-200 rounded-lg`}
    >
      {link.title === "Logout" ? (
        <button
        ...
        >
          ...
          <span className="text-sm  ml-2 font-semibold text-red-800 hover:text-red-700">
            {link.title}
          </span>
        </button>
      ) : (
        <Link
          to={link.to}
          className={`w-full flex items-center p-2 ${
            useMatch(link.to) ? "bg-blue-200 rounded-lg" : "bg-transparent"
          }`}
